...
leftpad xs n = if diff > 0
    then take diff (repeat ' ') ++ xs
    else xs
    where
        diff = n - length xs
...
countdown n = "Ready! " ++ helperCountdown n "" ++ "Liftoff!"
  where
    helperCountdown :: Integer -> String -> String
    helperCountdown 0 str = str
    helperCountdown k str = show k ++ "... " ++ helperCountdown (k-1) str
...
smallestDivisor n = smallestDivisorHelper n 2
  where
    smallestDivisorHelper n k = if mod n k /= 0 then
      smallestDivisorHelper n (k+1) else k
...
isPrime 0 = False
isPrime 1 = False
isPrime n = if n == smallestDivisor n then True else False
