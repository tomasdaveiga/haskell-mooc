...
swapIORefs ioRefA ioRefB = do
    contentA <- readIORef ioRefA
    contentB <- readIORef ioRefB
    writeIORef ioRefA contentB
    writeIORef ioRefB contentA 
...
hSelectLines h nums = do
    lins <- hFetchLines h
    return $ map (\idx -> lins !! (idx - 1)) nums
...
interact' f state = do
    ln <- getLine
    let (bl, newStr, newSt) = f (ln, state)
    putStrLn newStr
    if bl then interact' f newSt else return newSt
