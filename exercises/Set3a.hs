...
firstHalf str = if even (length str) 
  ...
  else take (div (length str) 2 + 1) str
...
joinToLength a xs = [first ++ last | first <- xs, last <- xs, length (first ++ last) == a]
...
multiApp f gs x = f $ go gs x
  where go [] x = []
        go (g:gs) x = g x : go gs x
...
interpreter commands = result
  where
    result = fst $ applyCommands commands ([], (0,0))
    applyCommands :: [String] -> ([String], (Integer, Integer)) -> ([String], (Integer, Integer))
    applyCommands cms (res, (x, y)) = case cms of
      [] -> (res, (x, y))
      (c:cs) -> applyCommands cs (applyCommand c (res, (x, y)))
    applyCommand cms (rs, (x,y)) = case cms of
      "up" -> (rs,(x, y+1))
      "down" -> (rs, (x, y-1))
      "left" -> (rs, (x-1, y))
      "right" -> (rs, (x+1, y))
      "printX" -> (rs ++ [show x], (x,y))
      "printY" -> (rs ++ [show y], (x,y))